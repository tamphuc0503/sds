...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not files_to_process:
            self.stdout.write(
                self.style.WARNING("No files found with null md5_content")
            )
            return
                    ...
                    if cur_md5 == md5:
                        ...
                        self.stdout.write(
                            self.style.HTTP_INFO(
                                f"Downloaded {local_file_path} with md5 {md5}"
                            )
                        )
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"MD5 mismatch: expected {cur_md5}, got {md5}"
                            )
                        )
    ...
    def _assign_md5_content(self, md5, local_file_path):
        ...
        try:
            ...
            if updated_count > 0:
                ...
                self.stdout.write(
                    self.style.WARNING(f"No records found with md5={md5}")
                )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error processing PDF for md5={md5}: {e}")
            )
            raise
    ...
    def _delete_local_file(self, file_path):
        ...
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                self.stdout.write(
                    self.style.SUCCESS(f"Deleted local file: {file_path}")
                )
                return True
            else:
                self.stdout.write(
                    self.style.WARNING(f"File not found, cannot delete: {file_path}")
                )
                return False
        except Exception as e:
    ...
    def download_file_to_memory(self, s3_file_path, bucket_name="sds"):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully downloaded {len(content)} bytes")
            )
            return content
...
def extract_layout_blocks(pdf_path):
    """
    Extract layout blocks from the first page of a PDF file.
    
    Args:
        pdf_path (str): Path to the PDF file
        
    Returns:
        list: List of dictionaries containing block bounding boxes and text lines
    """
...
def extract_first_page_text(pdf_path):
    ...
    try:
        ...
        text_blocks = first_page.get_text_blocks()
...
def process_pdf_to_md5(pdf_path):
    ...
    raw_text, text_blocks = extract_first_page_text(pdf_path)    
    print(f"Raw text extracted ({len(raw_text)} characters)")
    
    # Extract layout blocks for first page
    sentences = process_pdf_to_sentences(pdf_path=pdf_path)
    # Detect language
...
def process_pdf_to_sentences(pdf_path):
    blocks = extract_layout_blocks(pdf_path)
    all_sentences = []

    for block in blocks:
        merged_lines = merge_lines_smart(block["lines"])
        block_text = " ".join(merged_lines)
...
def merge_lines_smart(lines):
    ...
    for line in lines:
        ...
        # If previous line doesn't end sentence, merge
...
def merge_lines_with_sklearn_cluster():
    pass

def detect_section():
    """ Order by Y-Axis to get sentence.
    Loop each sentence and mark that sentence with regex (nlp in future).
    max_confidence = mark_sentence()
    if next sentence has confidence higher, select next sentence. and keep checking.
    if next sentence has confidence lower, select previous sentence.
    Stop
    """    
    pass